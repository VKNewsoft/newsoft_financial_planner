<?php helper('html'); ?>
<style>
.pcfr-shell .pcfr-mobile-list{display:none}.pcfr-shell .pcfr-mobile-item{border:1px solid #e8ece8;border-radius:1rem;padding:1rem;background:#fff;box-shadow:0 10px 22px rgba(39,60,48,.05)}.pcfr-shell .pcfr-mobile-loadmore{display:none}.pcfr-shell .pcfr-filter-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:.85rem}.pcfr-shell .pcfr-nav{display:flex;gap:.75rem;flex-wrap:wrap}.pcfr-shell .pcfr-nav .btn{border-radius:.7rem}.pcfr-shell .pcfr-mobile-meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.7rem;margin-top:.8rem}.pcfr-shell .pcfr-mobile-meta span{display:block;font-size:.76rem;color:#667085;margin-bottom:.15rem}.pcfr-shell .pcfr-table-wrap{width:100%;overflow:hidden}.pcfr-shell .pcfr-table-wrap .dataTables_wrapper{width:100%}.pcfr-shell .pcfr-table-wrap table{width:100%!important}.pcfr-shell .pcfr-table-wrap .dataTables_scrollHeadInner,.pcfr-shell .pcfr-table-wrap .dataTables_scrollHeadInner table,.pcfr-shell .pcfr-table-wrap .dataTables_scrollBody table{width:100%!important}@media(max-width:767.98px){.pcfr-shell .pcfr-table-desktop{display:none}.pcfr-shell .pcfr-mobile-list{display:grid;gap:.85rem}.pcfr-shell .pcfr-mobile-loadmore{display:inline-flex}.pcfr-shell .pcfr-filter-grid{grid-template-columns:1fr}}
</style>
<div class="result-page-shell pcfr-shell">
	<div class="result-page-hero">
		<div><div class="result-page-kicker">Finance / Report</div><h3 class="result-page-heading"><?= esc($title) ?></h3><p class="result-page-copy mb-0">Module report transaksi dengan filter lengkap, DataTable server-side, dan mobile card lazy-load.</p></div>
		<div class="pcfr-nav">
			<a href="<?= base_url('personal-cash-flow-report/transactions') ?>" class="btn btn-primary btn-sm">Daftar Transaksi</a>
	...
	</div><div class="mt-3 d-flex gap-2"><button type="submit" class="btn btn-primary btn-sm">Terapkan</button><a href="<?= base_url('personal-cash-flow-report/transactions') ?>" class="btn btn-outline-secondary btn-sm">Reset</a></div></form></div></div>
	<div class="card result-list-card"><div class="card-body p-0">
		<div class="result-list-toolbar"><div><h5 class="mb-1">Data Transaksi</h5><p class="mb-0 text-muted">Desktop memakai DataTable server-side, mobile memakai card/list dengan load more.</p></div></div>
...
<div id="pcfr-mode" class="d-none">transactions</div>
